...
int    g_iDisplayNumber, g_iDeathTime, g_iPlayerNum, g_iChapterTotal[2], g_iCumulativeTotal[2], g_iNumber[7], g_iKillSpecial[MAXPLAYERS+1], g_iHeadSpecial[MAXPLAYERS+1], g_iKillZombie[MAXPLAYERS+1], g_iDmgHealth[2][MAXPLAYERS+1];
...
public void OnPluginStart()
{
	...
	g_hPlayersNumber	= CreateConVar("l4d2_emshud_show_players_number", "127", "显示玩家数量信息(需要启用的功能数字相加,值为<0>时自动隐藏). 0=禁用, 1=连接, 2=闲置, 4=旁观, 8=丧尸, 16=女巫, 32=特感, 64=生还, 127=所有.", CVAR_FLAGS);
	...
	g_hTypeRanking		= CreateConVar("l4d2_emshud_ranking_Type", "255", "排行榜显示那些内容(需要启用的功能数字相加). 0=禁用, 1=状态, 2=血量, 4=特感, 8=爆头, 16=丧尸, 32=被黑, 64=友伤, 128=名字, 255=所有.", CVAR_FLAGS);
	g_hDispRanking		= CreateConVar("l4d2_emshud_ranking_disp", "63", "那些内容的值都为<0>时自动隐藏(需要自动隐藏的功能数字相加). 0=显示, 1=血量, 2=特感, 4=爆头, 8=丧尸, 16=被黑, 32=友伤, 63=所有.", CVAR_FLAGS);
...
void IsShowServersNumber()
{
	...
	HUDSetLayout(HUD_SCORE_1, HUD_FLAG_ALIGN_CENTER|HUD_FLAG_NOBG|HUD_FLAG_TEXT|HUD_FLAG_BLINK, g_sTotal);
