...
public void OnPluginStart()
{
	...
	l4d2_helicopter_size = CreateConVar("l4d2_helicopter_size", "1.0", "直升机尺寸 [0.5, 2.0]");
	l4d2_helicopter_speed = CreateConVar("l4d2_helicopter_speed", "500.0", "飞行速度 [100.0, 500.0]");
	l4d2_helicopter_gun_accuracy = CreateConVar("l4d2_helicopter_gun_accuracy", "1.0", "机枪精准度 [0.0, 1.0]");
	l4d2_helicopter_gun_damage = CreateConVar("l4d2_helicopter_gun_damage", "60", "机枪伤害 [1.0, 200.0]");
	l4d2_helicopter_password = CreateConVar("l4d2_helicopter_password", "3034", "指令密码' in chat");
	...
	l4d2_helicopter_fuel= CreateConVar("l4d2_helicopter_fuel", "2000.0", "Fuel amount in seconds [1.0, 2000.0]（燃油消耗量？）");
	l4d2_helicopter_bullet = CreateConVar("l4d2_helicopter_bullet", "1600", "机枪弹药量 [100.0, 1600.0]");
	l4d2_helicopter_bomb = CreateConVar("l4d2_helicopter_bomb", "400", "炸弹弹药量 [100.0, 400.0]");
	l4d2_helicopter_range = CreateConVar("l4d2_helicopter_range", "1000.0", "距离队伍最远距离 [100.0, 1600.0]");
...
public Action sm_h(int client, int args)
{
 	if(client>0 && IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		else
		{
			...
			else PrintHintText(client, "密码不正确捏");
		}
	}
...
void Fly(int client, int button, int flag, float intervual, float time)
{
	...
	if(time-RangeCheckTime[client]>1.0)
	{
		...
		if(IsAlone(client))
		{
			...
			if(tick<0)tick=0;
			PrintHintText(client, "你距离队伍太远啦 %d", tick);
		}
