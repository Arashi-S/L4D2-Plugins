...
public void Event_PlayerLedgeGrab(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(IsValidClient(client) && GetClientTeam(client) == 2)
		PrintToChatAll("\x04[Tips]\x03%s\x05脚底抹油，挂边了.", GetTrueName(client));//聊天窗提示.
}
...
void OnTakeDamageAlivePost(int victim, int attacker, int inflictor, float damage, int damagetype) 
{
	...
	if (IsValidClient(attacker)) 
	{
		...
		{
			case 2: 
				PrintToChatAll("\x04[Tips]\x03%s\x05彻底制服了\x03%s", GetTrueName(attacker), GetTrueName(victim));//聊天窗提示.
			case 3: 
				PrintToChatAll("\x04[Tips]\x03%s%s\x05彻底制服了\x03%s", g_sZombieName[GetEntProp(attacker, Prop_Send, "m_zombieClass") - 1], GetPlayerName(attacker), GetTrueName(victim));//聊天窗提示.
		}
	}
	else
	{
		if (IsValidEntity(attacker)) 
		{
			...
			if (damagetype & DMG_DROWN && GetEntProp(victim, Prop_Data, "m_nWaterLevel") > 1)
				PrintToChatAll("\x04[Tips]\x03%s\x05淹死了.", GetTrueName(victim));//聊天窗提示.
				...
				PrintToChatAll("\x04[Tips]\x03%s\x05试图尝试落地水.", GetTrueName(victim));//聊天窗提示.
			else if (strcmp(classname, "worldspawn") == 0 && damagetype == 131072)
				PrintToChatAll("\x04[Tips]\x03%s\x05流血过多而死.", GetTrueName(victim));//聊天窗提示.
			else if (strcmp(classname, "infected") == 0)
				PrintToChatAll("\x04[Tips]\x03丧尸\x05杀死了\x03%s", GetTrueName(victim));//聊天窗提示.
			else if (StrEqual(classname, "witch", false))
				PrintToChatAll("\x04[Tips]\x03%s\x05杀死了\x03%s", GetWitchName(attacker), GetTrueName(victim));//聊天窗提示.
			else if (strcmp(classname, "insect_swarm") == 0)
				PrintToChatAll("\x04[Tips]\x05玩家\x03%s\x05酸死了.", GetTrueName(victim));//聊天窗提示.
			else
				PrintToChatAll("\x04[Tips]\x03%s\x05已死亡.", GetTrueName(victim));//聊天窗提示.
		}
	}
}
...
public void Event_Incapacitate(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (IsValidClient(client) && GetClientTeam(client) == 2)
	{
		if (IsValidClient(attacker))
		{
			...
			{
				case 2: 
					PrintToChatAll("\x04[Tips]\x03%s\x05制服了\x03%s", GetTrueName(attacker), GetTrueName(client));//聊天窗提示.
				case 3: 
					PrintToChatAll("\x04[Tips]\x03%s%s\x05制服了\x03%s", g_sZombieName[GetEntProp(attacker, Prop_Send, "m_zombieClass") - 1], GetPlayerName(attacker), GetTrueName(client));//聊天窗提示.
			}
		}
		else
		{
			if (IsValidEntity(entity)) 
			{
				...
				if (damagetype & DMG_DROWN && GetEntProp(client, Prop_Data, "m_nWaterLevel") > 1)
					PrintToChatAll("\x04[Tips]\x03%s\x05晕倒了.", GetTrueName(client));//聊天窗提示.
				else if (damagetype & DMG_FALL)
					PrintToChatAll("\x04[Tips]\x03%s\x05从不应该掉下来的地方掉了下来.", GetTrueName(client));//聊天窗提示.
				else if (strcmp(classname, "infected") == 0)
					PrintToChatAll("\x04[Tips]\x03丧尸\x05制服了\x03%s", GetTrueName(client));//聊天窗提示.
				else if (StrEqual(classname, "witch", false))
					PrintToChatAll("\x04[Tips]\x03%s\x05制服了\x03%s", GetWitchName(entity), GetTrueName(client));//聊天窗提示.
				else if (strcmp(classname, "insect_swarm") == 0)
					PrintToChatAll("\x04[Tips]\x05玩家\x03%s\x05在痰里蹦跶没两下就倒了.", GetTrueName(client));//聊天窗提示.
				else
					PrintToChatAll("\x04[Tips]\x03%s\x05倒下了.", GetTrueName(client));//聊天窗提示.
			}
		}
	}
}
...
char[] GetWitchName(int iWitchid)
{
	...
	if(GetWitchNumber(iWitchid) == 0) 
		...
		FormatEx(clName, sizeof(clName), "Witch(%d)", GetWitchNumber(iWitchid));
