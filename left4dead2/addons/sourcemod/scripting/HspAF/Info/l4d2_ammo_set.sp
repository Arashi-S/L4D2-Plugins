...
public Action l4d2_Offammosets(int client, int args)
{
	if(bCheckClientAccess(client))
	{
		switch(G_l4d2_ammo)
		{
			case 0:
			{
				PrintToChat(client, "\x04[AMMO]\x05更多后备弹药已禁用,请在CFG中设为1启用.");
			}
			case 1,2:
			{
		...
		PrintToChat(client, "\x04[AMMO]\x05你在服务器什么权限没点B数吗.");
...
void l4d2_ammosetStartDelays_prompt()
{
	if (l4d2_Rloadammoset)
	{
		if (l4d2_Theammoset)
		{
			PrintToChatAll("\x04[AMMO]\x05已开启\x03更多\x05后备弹药.");
		}
		else
		{
			PrintToChatAll("\x04[AMMO]\x05已开启\x03无限\x05后备弹药.");
		}
	}
	else
	{
		PrintToChatAll("\x04[AMMO]\x05已关闭\x03更多\x05后备弹药.");
	}
}
