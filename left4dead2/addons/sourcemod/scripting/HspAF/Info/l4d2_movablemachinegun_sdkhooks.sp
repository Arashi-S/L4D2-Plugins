...
public OnPluginStart()
{
	...
	sm_movableminigunblock = CreateConVar("l4d2_movablemachinegun_block", "0","移动机枪时禁止攻击",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 1.0);
	sm_movableminigundist = CreateConVar("l4d2_movablemachinegun_distance", "200.0","阻止在另一挺机枪这个范围内部署另一挺机枪（0.0 禁用）",FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 999999999.0);
	sm_movableminigunspeed = CreateConVar("l4d2_movablemachinegun_speed", "0.8","移动迷你枪时的行走速度 1.0 = 正常速度，< 0 = 不修改",FCVAR_PLUGIN|FCVAR_NOTIFY);
...
public Action:OnEntityUse(entity, activator, caller, UseType:type, Float:value)
{
	if (type ==Use_Toggle)
	{
		PrintToChat(caller,"\x01[\x03MMG\x01]\x03你可以通过 \x04瞄准重机枪\x03 并按shift + \x04   鼠标右键来移动它 \x03Button Together");
	}
}
		...
		if (parenting[client]!=0)
		{
			...
			if (Dist >0.0)
			{
				...
				while ((gun = FindEntityByClassname2(gun, "prop_minigun")) != -1)
				{
					...
					{
						PrintToChat(client,"\x01[\x03MMG\x01]\x03你离另一挺机枪太近了，远离它来部署你的机枪 (%f/%f)",GetVectorDistance(PVec, MVec),Dist);
						return;
					}
				}
				if (l4d2)
