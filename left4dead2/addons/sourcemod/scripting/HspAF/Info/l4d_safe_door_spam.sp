...
public void OnPluginStart()
{
	...
	g_hCvarModes =		CreateConVar(	"l4d_safe_spam_modes",			"",				"在这些游戏模式中启用插件(用逗号隔开，无空格。)留空=全部模式.", CVAR_FLAGS);
	g_hCvarModesOff =	CreateConVar(	"l4d_safe_spam_modes_off",		"",				"在这些游戏模式中关闭插件(用逗号隔开，无空格。)留空=无.", CVAR_FLAGS);
	g_hCvarModesTog =	CreateConVar(	"l4d_safe_spam_modes_tog",		"0",			"在这些游戏模式中打开插件。 0=全部，1=合作，2=生存，4=对战，8=清道夫。 可将数字相加.", CVAR_FLAGS);
	g_hCvarFallTime =	CreateConVar(	"l4d_safe_spam_fall_time",		"0.0",			"0.0=关闭。 回合开始后（通过 l4d_safe_spam_lock* 和 l4d_safe_spam_lock* cvars 解锁后）锁定的安全门自动打开的等待时间， 除非手动打开.", CVAR_FLAGS);
	g_hCvarGlow =		CreateConVar(	"l4d_safe_spam_glow",			"255 0 0",		"0=Off. 颜色选择RGB，用空格隔开.", CVAR_FLAGS);
	g_hCvarHint =		CreateConVar(	"l4d_safe_spam_hint",			"0",			"0=Off. 1=提示谁打开了安全门。 2=当安全室门自动解锁时提示（_touch 和 _lock cvars）。 3=两者.", CVAR_FLAGS);
	g_hCvarHints =		CreateConVar(	"l4d_safe_spam_hints",			"1",			"当玩家尝试打开安全门时，安全门锁定时间提示应显示在哪里. 1=聊天栏. 2=提示栏.", CVAR_FLAGS);
	g_hCvarLast =		CreateConVar(	"l4d_safe_spam_last",			"0",			"回合开始时的最终门状态：0=使用地图默认值。 1=关闭终点门。 2=打开终点门.", CVAR_FLAGS);
	g_hCvarLock =		CreateConVar(	"l4d_safe_spam_lock",			"0.0",			"0.0=Off. 回合开始时安全门会锁定多长时间，单位秒.", CVAR_FLAGS);
	g_hCvarLock2 =		CreateConVar(	"l4d_safe_spam_lock_2",			"0.0",			"0.0=Off. 在第二回合安全门锁定多长时间，单位秒.", CVAR_FLAGS);
	g_hCvarTouch =		CreateConVar(	"l4d_safe_spam_touch",			"0.0",			"0.0=Off. 当玩家尝试打开上锁的安全门之后变为多长时间后解锁.", CVAR_FLAGS);
	g_hCvarTouch2 =		CreateConVar(	"l4d_safe_spam_touch_2",		"0.0",			"0.0=Off. 在第二回合当玩家尝试打开上锁的安全门之后变为多长时间后解锁.", CVAR_FLAGS);
	g_hCvarOpen =		CreateConVar(	"l4d_safe_spam_open",			"2",			"0=Off, 1=打开第一个安全室门后保持打开状态，2=打开后使第一个安全室门掉落.", CVAR_FLAGS);
	g_hCvarPhysics =	CreateConVar(	"l4d_safe_spam_physics",		"3.0",			"0.0=倒下的门总是有物理效果 | 安全门倒下后多少秒之后出现物理效果.", CVAR_FLAGS);
	if( g_bLeft4Dead2 )
		g_hCvarSkin =	CreateConVar(	"l4d_safe_spam_skin",			"0",			"0=地图默认值。 1=经典。 2=临死一博 | 起点和终点安全室使用哪种门模型.", CVAR_FLAGS);
	g_hCvarTimeClose =	CreateConVar(	"l4d_safe_spam_time_close",		"1.0",			"fw翻译官并不知该如何翻译：How many seconds to block after closing the last saferoom door.", CVAR_FLAGS);
	g_hCvarTimeOpen =	CreateConVar(	"l4d_safe_spam_time_open",		"3.0",			"fw翻译官并不知该如何翻译：How many seconds to block after opening the last saferoom door.", CVAR_FLAGS);
	g_hCvarType =		CreateConVar(	"l4d_safe_spam_type",			"3",			"fw翻译官并不知该如何翻译：0=Off. When the last saferoom door is used enable the timeout on: 1=Open, 2=Close, 3=Both.", CVAR_FLAGS);
