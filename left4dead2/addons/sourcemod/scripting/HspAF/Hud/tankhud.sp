...
public void OnPluginStart()
{
	...
	g_hPanelTitle = CreateConVar("tank_hud_title", "HspAF Tank Hud", "Tank 状态面板标题", CVAR_FLAG);
...
public Action Cmd_TankHud(int client, int args)
{
	if (!hiddenPanel[client])
	{
		...
		CPrintToChat(client, "{G}<TankHUD>：{W}Tank HUD 现在是 {LG}关闭状态");
		return Plugin_Continue;
	}
	hiddenPanel[client] = false;
	CPrintToChat(client, "{G}<TankHUD>：{W}Tank HUD 现在是 {LG}开启状态");
	return Plugin_Continue;
}
