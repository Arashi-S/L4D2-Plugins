...
float g_fMaxNonAngryDist, g_fNonAngryTime, g_fTankClawRange, g_fMapMaxFlow;
...
public void OnPluginStart() {
	...
	g_hCvarNonAngryTime = CreateConVar("l4d_TankAntiStuck_non_angry_time",				"15.0",		"如果在指定的多少秒坦克出现但没有行动(响起BGM)则自动传送坦克(0=禁用).", CVAR_FLAGS);
	g_hCvarTankDistanceMax = CreateConVar("l4d_TankAntiStuck_tank_distance_max",		"1500.0",	"坦克与最近的幸存者允许的最远距离(BGM响起时),如果超过这个距离则传送坦克(0=禁用).", CVAR_FLAGS);
	...
	g_hCvarRusherDist = CreateConVar("l4d_TankAntiStuck_rusher_dist",					"800.0",	"离坦克这么远被认为是在跑图.", CVAR_FLAGS);
	g_hCvarRusherCheckTimes = CreateConVar("l4d_TankAntiStuck_rusher_check_times",		"2",		"检测多少次玩家如果在跑图,下一次达到设置检测时间后就执行惩罚传送.", CVAR_FLAGS);
	g_hCvarRusherCheckInterv = CreateConVar("l4d_TankAntiStuck_rusher_check_interval",	"3.0",		"多少秒检测一次玩家是否跑图.", CVAR_FLAGS);
				...
				if (distance > g_hCvarRusherDist.FloatValue) {
					if (g_iRushTimes[i] >= g_hCvarRusherCheckTimes.IntValue) {
						...
						if (!IsPos_StuckTank(postank, tank)) {
							...
							g_iRushTimes[i] = 0;
							PrintToChatAll("\x04[提示]\x05反跑男启动\x03坦克%s\x05传送到跑男\x03%s\x05身边.", GetPlayerName(tank), GetTrueName(i));
						}
						pos[0] += 20.0;
						pos[2] -= 20.0;
					}
...
void TeleportToObject(int client) {
	...
	if (target > 0 && IsPlayerAlive(client) && GetClientTeam(client) == 3 && GetEntProp(client, Prop_Send, "m_zombieClass") == 8) {
		...
		PrintToChatAll("\x04[提示]\x05Tank防卡启动\x03坦克%s\x05%s传送到幸运玩家%s%s\x03%s\x05附近.", 
...
static int GetAheadSurvivor() {
	...
	RoundToCeil(flow / g_fMapMaxFlow * 100) > 0 ? flowList.Sort(Sort_Descending, Sort_Float) : flowList.Sort(Sort_Ascending, Sort_Float);
