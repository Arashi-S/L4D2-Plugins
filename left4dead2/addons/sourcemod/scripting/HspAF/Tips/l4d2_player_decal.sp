...
public Action PlayerDecal(const char[] te_name, const int[] Players, int numClients, float delay)
{
	...
	if(IsValidClient(client))
	{
		...
		{
			case 2:
				PrintToChatAll("\x04[提示]\x03%s(%s)\x05在墙上喷了点什么.", GetPlayerName(client), GetPlayerModel(client));
			case 3:
				PrintToChatAll("\x04[提示]\x03%s(%s)\x05在墙上喷了点什么.", GetPlayerName(client), g_sZombieName[GetEntProp(client, Prop_Send, "m_zombieClass") - 1]);
			default:
				PrintToChatAll("\x04[提示]\x03%s\x05在墙上喷了点什么.", GetPlayerName(client));
		}
	}
