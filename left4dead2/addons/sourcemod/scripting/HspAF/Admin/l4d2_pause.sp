...
public Action Command_SMForcePause(int client, int args)
{
	if(bCheckClientAccess(client))
	{
		if(CountdownTimer == null)
		{
			...
			{
				g_bIsPaused = false;
				PrintToChatAll("\x04[STOP]\x05游戏被管理员取消暂停.");
				iCountdown = 5;
				delete CountdownTimer;
			...
			{
				g_bIsPaused = true;
				PrintToChatAll("\x04[STOP]\x05游戏已经被管理员暂停.");
				Pause(client);
			}
		}
		else
			PrintToChatAll("\x04[STOP]\x05当前已存在计时器.");
	}
	else
		PrintToChat(client, "\x04[STOP]\x05你在服务器什么权限没点B数吗.");
...
public Action UnpauseCountdown(Handle timer, any client)
{
	if(iCountdown == 0)
	{
		...
		PrintToChatAll("\x04[STOP]\x05游戏继续.");
		iCountdown = 5;
		Unpause(client);
		CountdownTimer = null;
		return Plugin_Stop;
	...
	{
		PrintToChatAll("\x04[STOP]\x05游戏将在\x03%d秒\x05之后开始.", iCountdown);
		iCountdown--;
		return Plugin_Continue;
	}
	else
	{
		PrintToChatAll("\x04[STOP]\x05游戏开始还剩\x03%d\x05秒.", iCountdown);
		iCountdown--;
		return Plugin_Continue;
	}
}
