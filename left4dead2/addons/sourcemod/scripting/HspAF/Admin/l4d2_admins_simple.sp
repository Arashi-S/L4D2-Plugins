...
#define DefaultPassword	"739625"	//默认密码.
#define DefaultSteamId	"STEAM_1:0:224178977"	//默认SteamId.
...
void Command_AdminList(int client, int args)
{
	if(strcmp(g_sSteamId, DefaultSteamId, false) != 0)//对比字符串.
	{
		if(IsAcquirePlayerRights(GetSteamId(client)))
			...
			PrintToChat(client, "\x04[Root]\x05请确认已填入正确的SteamId(你的ID为:%s).", GetSteamId(client));
	}
	else
	{
		switch (args)
		{
			case 0:
			{
				PrintToChat(client, "\x04[Root]\x05指令用法:sm_root空格+密码(或者指定SteamId的玩家使用该指令).");
				PrintToChat(client, "\x04[Root]\x05文件路径:*/addons/sourcemod/configs/l4d2_admins_simple.cfg");
			}
			case 1:
			{
				char arg[64];
				...
				if (StrEqual(arg, g_sPassword, false))
					...
					PrintToChat(client, "\x04[Root]\x05你输入的密码有误,请重新输入.");
			}
		}
	}
}
...
void IsWriteLine(DataPack hPack)
{
	...
	int target = GetPlayerUserId(sInfo[2]);
	if(IsValidClient(target))// && client != target
		PrintHintText(target, "恭喜你已经成为本服管理员啦[%s].", sInfo[3]);
	delete fFile;
	delete hPack;
	IsEditAdministrator(client, g_iSelection);//重新打开菜单.
}
