...
public void OnPluginStart() {
	...
	CreateConVar("ry_version", PLUGIN_VERSION, "Give Item Menu plugin version.", FCVAR_NOTIFY|FCVAR_DONTRECORD);

	RegAdminCmd("sm_ry", cmdRy, ADMFLAG_CHEATS, "ry");
...
Action cmdRy(int client, int args) {
...
void Ry(int client) {
	Menu menu = new Menu(Ry_MenuHandler);
	menu.SetTitle("Cheat命令");
	menu.AddItem("w", "生成武器");
	menu.AddItem("i", "生成物品");
	menu.AddItem("z", "生成感染");
	//root权限玩家可用
	if (bCheckClientAccess(client) && GetClientImmunityLevel(client) > 66)
		menu.AddItem("m", "杂项功能");
	//免疫达到99同时root权限玩家可用
...
int Ry_MenuHandler(Menu menu, MenuAction action, int client, int param2) {
	switch (action) {
