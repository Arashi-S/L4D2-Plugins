...
public Action MenuMode(int client, int args)
{
	if(bCheckClientAccess(client))
		...
		PrintToChat(client, "\x04[WARNING]\x05你在服务器什么权限没点B数吗.");
...
void GetModeMenu(int client, int args)
{
	...
	{
		case 0:
			PrintToChat(client, "\x05[WARNING]\x03更换游戏模式已禁用,请在CFG中设为1启用.");
		case 1:
	...
	switch (args)
	{
		...
		case 1:
		{
			...
			if(strcmp(g_sMode, GetGameMode(), false) == 0)
				PrintToChat(client, "\x04[WARNING]\x05当前已是\x04(\x03%s\x04)\x05模式.", GetModeName());
			else
			{
				SetConVarString(FindConVar("mp_gamemode"), g_sMode);
				
				if (!StrEqual(g_sMode, GetGameMode(), false) == true)
					PrintToChat(client, "\x04[Mode]\x05请确认你输入的模式代码\x04[\x03%s\x04]\x05是否正确.", g_sMode);
				else
				{
					if(g_hGameMode == null)
					{
						PrintToChatAll("\x04[Mode]\x05游戏模式更改为\x04(\x03%s\x04)\x04,\x05将在\x03%.1f秒\x05后重启当前章节.", GetModeName(), g_fChangeTime);
						PrintHintTextToAll("[Mode] 游戏模式更改为(%s),期间黑屏,请等待数秒.", GetModeName());
						g_hGameMode = CreateTimer(g_fChangeTime, DelaySetGameMode);//延迟重启当前章节
					}
					else
						PrintToChat(client, "\x04[WARNING]\x05当前正在更改游戏模式,请勿重复设置游戏模式.", GetModeName());
				}
			}
		}
	}
...
void IsRestartGameMenu(int client, char[] g_sModeName)
{
	if(g_hGameMode == null)
	{
		...
		PrintToChat(client, "\x04[WARNING]\x05当前正在重启当前章节.");
}
...
void IsForceRestartGame()
{
	delete g_hGameMode;
	g_hGameMode = CreateTimer(g_fChangeTime, DelaySetGameMode);//延迟重启当前章节
	PrintHintTextToAll("[WARNING] 服务器将在 %.1f秒 后重启当前章节.", g_fChangeTime);
	PrintToChatAll("\x04[WARNING]\x05服务器将在\x03%.1f秒\x05后重启当前章节.", g_fChangeTime);
}
	...
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			if(strcmp(sInfo[0], GetGameMode(), false) == 0)
			{
				...
				PrintToChat(client, "\x04[WARNING]\x05当前已是\x04(\x03%s\x04)\x05模式.", sInfo[1]);
			}
			else
			{
				if(g_hGameMode == null)
				{
					...
					PrintHintTextToAll("[Mode] 游戏模式更改为(%s),期间黑屏,请等待数秒.", sInfo[1]);
					PrintToChatAll("\x04[Mode]\x05游戏模式更改为\x04(\x03%s\x04)\x04,\x05将在\x03%.1f秒\x05后重启当前章节.", sInfo[1], g_fChangeTime);
				}
				else
					PrintToChat(client, "\x04[WARNING]\x05当前正在更改模式,请勿重复设置游戏模式.");
			}
		}
		case MenuAction_Cancel:
			if(itemNum == MenuCancel_ExitBack)
