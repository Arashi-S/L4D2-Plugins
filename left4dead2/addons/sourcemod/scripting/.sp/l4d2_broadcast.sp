...
public Action l4d2_broadcast_don(int client, int args)
{
	if(bCheckClientAccess(client))
	{
		...
		PrintToChat(client, "\x04[提示]\x05你在服务器什么权限没点B数吗.");
...
public void Event_PlayerHurt(Event event, const char[] name, bool dontBroadcast)
{
	if (Hbroadcast_don != 0 && Hbroadcast_don_Black_Gun != 0 && l4d2_broadcast_ff)
	{
		...
		{
			if (client == attacker)
				PrintToChat(attacker, "\x04[???]\x05请勿自残\x04!");
			else
			{
				if (fire_counts[attacker][client])
				{
					...
					PrintToChat(attacker, "\x04[???]\x05你打中了了\x03%s\x04%s\x04.", GetTrueName(client), g_sHitName[iHitGroup]);
					PrintToChat(iBot != 0 ? iBot : client, "\x04[???]\x03%s\x05攻击了你\x04%s\x04.", GetTrueName(attacker), g_sHitName[iHitGroup]);
				}
			}
			for (int i = 1; i <= MaxClients; i++)
