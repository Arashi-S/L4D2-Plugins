...
public Action IsShowTipsTimer(Handle timer, any client)
{
	if ((client = GetClientOfUserId(client)))
	{
		if (IsClientInGame(client))
		{
			...
			{
				case 1,3:
					PrintToChat(client, "\x04[ZS]\x05输入指令\x03!zs\x05或\x03!kill\x05或\x03自杀\x05可自杀.");//聊天窗提示.
				case 2,4:
				{
					switch (g_iSuicide)
					{
						case 1:
							PrintToChat(client, "\x04[ZS]\x05倒地或挂边时输入指令\x03!zs\x05或\x03!kill\x05或\x03自杀\x05可自杀.");//聊天窗提示.
						case 2:
							PrintToChat(client, "\x04[ZS]\x05输入指令\x03!zs\x05或\x03!kill\x05或\x03自杀\x05可自杀.");//聊天窗提示.
						default:
							PrintToChat(client, "\x04[ZS]\x05输入指令\x03!zs\x05或\x03!kill\x05或\x03自杀\x05可自杀.");//聊天窗提示.
	...
	if ((client = GetClientOfUserId(client)))
		if(IsClientInGame(client) && !IsFakeClient(client))
			if(g_iSuicide > 0)
				...
				PrintToChat(client, "\x04[ZS]\x05玩家自杀指令未启用.");
}
		...
		switch (GetClientTeam(client))
		{
			case 1:
			{
				...
				if (iBot != 0)
					...
					PrintToChat(client, "\x04[ZS]\x05旁观者你自杀个寄吧.");
			}
			case 2:
		...
		switch (g_iSuicide)
		{
			case 1:
				if (!IsPlayerState(client))
					...
					PrintToChat(victim, "\x04[ZS]\x05该指令只限倒地或挂边的%s使用.", g_sTeam);
			case 2:
				IsForceSuicide(client, victim, g_sName, g_sTeam);//执行玩家死亡代码.
		}
	}
	else
		PrintToChat(victim, "\x04[ZS]\x05你都嘎了自杀个寄吧.");
}
...
void IsForceSuicide(int client, int victim, char[] g_sName, char[] g_sTeam)
{
	if (IsPlayerAlive(client))
	{
		...
		PrintToChatAll("\x04[ZS]\x05(\x04%s\x05)\x03%s\x05突然感觉时间再无留恋，选择了自杀.", g_sTeam, g_sName);
	}
	else
		PrintToChat(victim, "\x04[ZS]\x05你都嘎了自杀个寄吧.");
}
