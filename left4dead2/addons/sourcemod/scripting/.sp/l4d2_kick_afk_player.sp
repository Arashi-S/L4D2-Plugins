...
public void OnPluginStart()
{
	...
	Kickplayer	= CreateConVar("l4d2_afk_player_kick_time","600","设置普通玩家闲置超时踢出的时间/秒(不建议低于300秒).", FCVAR_NOTIFY);
	Kickadmin	= CreateConVar("l4d2_afk_player_kick_time_admin","1200","设置管理员闲置超时踢出的时间/秒(不建议低于300秒).", FCVAR_NOTIFY);
...
public Action l4d2_timer_kick_switch(int client, int args)
{
	if(bCheckClientAccess(client))
	{
		switch(g_Kickafk)
		{
			case 0:
			{
				PrintToChat(client, "\x04[AFK]\x05普通玩家闲置超时踢出已禁用,请在CFG中设为1启用.");
			}
			case 1,2:
			{
				if(l4d2_timer_kick)
				{
					...
					if(g_Kickafk == 1)
						PrintToChatAll("\x04[AFK]\x03已关闭\x05普通玩家闲置超时踢出.");
					else if(g_Kickafk == 2)
						PrintToChatAll("\x04[AFK]\x03已关闭\x05普通玩家和管理员闲置超时踢出.");
				}
				else
				{
					forkickafkplayer();
					...
					if(g_Kickafk == 1)
						PrintToChatAll("\x04[AFK]\x03已开启\x05普通玩家闲置超时踢出.");
					else if(g_Kickafk == 2)
						PrintToChatAll("\x04[AFK]\x03已开启\x05普通玩家和管理员闲置超时踢出.");
				}
			}
		}
	}
	else
		PrintToChat(client, "\x04[AFK]\x05你在服务器什么权限没点B数吗.");
			...
			case 1:
			{
				...
				else
				{
					if (KickLookOnPlayer[client] >= g_kickplayer)
					{
						...
						PrintToChatAll("\x04[AFK]\x03%N\x05状态空闲超过\x03%s\x05而被服务器踢出.", client, sRunTime);//聊天窗AFK
						kickPlayerTimer[client] = null;
						KickLookOnPlayer[client] = 0;
						return Plugin_Stop;
					}
					else
					{
						...
						PrintHintText(client, "我知道你可能坏肚子了之类的，你有 %s 的时间处理的你的要事.", sRunTime);//屏幕中下AFK.
					}
