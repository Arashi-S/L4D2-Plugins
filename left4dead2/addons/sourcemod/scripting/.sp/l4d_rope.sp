...
public void OnPluginStart() { 	 
	...
	Plugin_enabled =	CreateConVar("rope_enabled", "1", "插件开关'", FCVAR_NOTIFY);
	Rope_damage =		CreateConVar("rope_damage", "10", "rope damage when attack infected", FCVAR_NOTIFY);
 	Rope_distance =		CreateConVar("rope_distance", "900.0", "绳索最大距离", FCVAR_NOTIFY);
	...
	Announce_types =	CreateConVar("rope_announce_types", "1", "公布位置 1=中心 2=聊天 4=提示 7=全部。 把你想要的加在一起", FCVAR_NOTIFY);
	Access =			CreateConVar("rope_access", "f", "允许这些权限的玩家使用绳索 留空=全部", FCVAR_NOTIFY);
	Allow_infected =	CreateConVar("rope_allow_infected", "0", "是否允许特感团队使用绳索？", FCVAR_NOTIFY);
