...
public void OnPluginStart()
{
    ...
    g_hCvar_Team            = CreateConVar("l4d_throwable_announcer_team", "1", "抛出投掷物的Grenade消息发送给那些团队. 0=没有, 1=幸存者, 2=感染者, 4=旁观者, 8=幸存者NPC. 如需启用多个把数字加起来. 例如: 3=幸存者+感染者.", CVAR_FLAGS, true, 0.0, true, 15.0);
    g_hCvar_Self            = CreateConVar("l4d_throwable_announcer_self", "1", "抛出投掷物的Grenade消息发送给使用者? 0=关闭, 1=开启.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_Molotov         = CreateConVar("l4d_throwable_announcer_molotov", "1", "每次抛出燃烧瓶时Grenade. 0=关闭, 1=开启.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_Pipebomb        = CreateConVar("l4d_throwable_announcer_pipebomb", "1", "每次抛出土制炸弹时Grenade. 0=关闭, 1=开启.", CVAR_FLAGS, true, 0.0, true, 1.0);
    if (g_bL4D2)
        g_hCvar_Vomitjar    = CreateConVar("l4d_throwable_announcer_vomitjar", "1", "每次抛出胆汁罐时Grenade. 0=关闭, 1=开启.", CVAR_FLAGS, true, 0.0, true, 1.0);
    ...
    switch (type)
    {
        case TYPE_MOLOTOV:
        {
            ...
            for (int client = 1; client <= MaxClients; client++)
            {
                ...
                PrintToChat(client, "\x04[Grenade]\x03%N\x05扔了个火瓶!", attacker);
            }
        }
        ...
        case TYPE_PIPEBOMB:
        {
            ...
            for (int client = 1; client <= MaxClients; client++)
            {
                ...
                PrintToChat(client, "\x04[Grenade]\x03%N\x05投掷了派棒!", attacker);
            }
        }
        ...
        case TYPE_VOMITJAR:
        {
            ...
            for (int client = 1; client <= MaxClients; client++)
            {
                ...
                PrintToChat(client, "\x04[Grenade]\x03%N\x05投掷了胆汁!", attacker);
            }
        }
    }
}
