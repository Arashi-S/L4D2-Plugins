...
public void OnPluginStart()
{
	...
	g_hPassword = CreateConVar("l4d2_admins_simple", "739625", "设置!root指令的密码(指令和密码不会显示到聊天窗).", FCVAR_NOTIFY);
...
void GetPlayersContent(int client, int args)
{
	if(g_sSteamId[0] != '\0')
	{
		if(AcquirePlayerRights(GetPlayersSteamId(client)))
			...
			PrintToChat(client, "\x04[WARNING]\x05你想干什么？？？.");
	}
	else
	{
		switch (args)
		{
			case 0:
			{
				PrintToChat(client, "\x04[Root]\x05用法:!root空格+密码(密码在配置文件l4d2_admins_simple.cfg里查看或修改).");
			}
			case 1:
			{
				char arg[64];
		...
		while(!fFile.EndOfFile())
		{
			...
			if(StrContains(szLine, sInfo[0]) == -1)//对比字符串.
			{
				...
				if(IsValidClient(target) && client != target)
					PrintHintText(target, "你干坏事了，服管移除了你的管理员权限.");
			}
		}
		delete fFile;
		delete fTempFile;
