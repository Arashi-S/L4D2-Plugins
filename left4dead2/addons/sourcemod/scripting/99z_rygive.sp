...
public void OnPluginStart() {
	...
	CreateConVar("add_version", PLUGIN_VERSION, "Give Item Menu plugin version.", FCVAR_NOTIFY|FCVAR_DONTRECORD);

	HookEvent("player_disconnect", Event_PlayerDisconnect, EventHookMode_Pre);
	RegAdminCmd("sm_add", cmdAdd, ADMFLAG_ROOT, "add");
...
public void OnClientPostAdminCheck(int client) {
	...
	if (!g_smSteamIDs.ContainsKey(sSteamID))
		KickClient(client, "服务器调试中...欢迎下次光临捏");
}
...
void Add(int client) {
	Menu menu = new Menu(Add_MenuHandler);
	menu.SetTitle("多功能插件:");
	menu.AddItem("w", "武器");
	menu.AddItem("i", "物品");
	menu.AddItem("z", "感染");
...
int Add_MenuHandler(Menu menu, MenuAction action, int client, int param2) {
	switch (action) {
