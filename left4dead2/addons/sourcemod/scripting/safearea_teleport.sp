...
public void OnPluginStart()
{
	...
	g_hSafeAreaFlags =		CreateConVar("st_enable",		"1",	"在哪里启用传送? (1=终点安全屋, 2=终点救援车, 3=两者)", CVAR_FLAGS);
	g_hSafeAreaType =		CreateConVar("st_type",			"1",	"倒计时结束后没有进入安全区的幸存者如何处理. (1=传送, 2=处死).", CVAR_FLAGS);
	g_hSafeAreaTime =		CreateConVar("st_time",			"30",	"设置倒计时的时间/秒.", CVAR_FLAGS);
	...
	if(g_bIsSacrificeFinale)
	{
		if(!iGetMissionWipes())
		{
			if(g_bTranslation)
				...
				PrintToChatAll("\x05该地图为牺牲结局，不启用落后传送");
		}
...
void  OnStartTouch(const char[] output, int caller, int activator, float delay)
{
	...
	if(iReached < iParam)
	{
		if(g_bTranslation)
			...
			vPrintHintToSurvivor("%d名生还者已到达终点(需要%d名)", iReached, iParam);
		return;
	}
...
Action tmrCountdown(Handle timer)
{
	if(g_iCountdown > 0)
	{
		if(g_bTranslation)
		{
			...
			vPrintHintToSurvivor("%d 秒后%s未到达终点的生还者", g_iCountdown--, g_iSafeAreaType == 1 ? "传送" : "处死");
