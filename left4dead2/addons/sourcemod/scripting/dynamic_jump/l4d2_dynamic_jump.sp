...
bool JumpAcrossGap(int entity, const float landingGoal[3], float velocity[3] = NULL_VECTOR)
{
	...
	any body = INextBot_GetBodyInterface(nextbot);
	if ( !IBody_StartActivity(body, 661, 0) ) // 661 - ACT_JUMP
		return false;
	...
	StoreToAddressOffsVector(locomotion, 0x6C, ahead);
	StoreToAddressOffsVector(locomotion, 0x94, {0.0, 0.0, 0.0});
	
	StoreToAddressOffs(locomotion, 0xC0, 0x01, NumberType_Int8);
	StoreToAddressOffs(locomotion, 0xC1, 0x01, NumberType_Int8);
	StoreToAddressOffs(locomotion, 0xD4, 0x00, NumberType_Int8);
